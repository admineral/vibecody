...
export default function IgnoreList({ isOpen, onToggle }: IgnoreListProps) {
  ...
  return (
    <div className="border-b border-sidebar-border bg-sidebar-accent/50">
      {/* Header */}
      ...
      {ignoredFilesArray.length === 0 ? (
        <div className="p-4 text-center">
          <Eye className="w-8 h-8 mx-auto text-muted-foreground/60 mb-2" />
          <p className="text-sm text-muted-foreground">No files ignored</p>
          ...
          <ScrollArea className={`${isExpanded ? 'h-64' : 'h-32'} px-3`}>
            <div className="space-y-1">
              {ignoredFilesArray.map((filePath) => (
                <div
                  key={filePath}
                  className="flex items-center justify-between p-2 bg-muted/50 rounded border hover:bg-muted group"
                >
