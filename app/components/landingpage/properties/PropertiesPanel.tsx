    ...
    return (
      <div className="h-full flex flex-col bg-gray-900 p-6">
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <h3 className="text-lg font-medium text-gray-200">No component selected</h3>
      ...
      <div className="px-4 py-3 border-b border-gray-700 flex items-center justify-between bg-gray-900">
        <div>
          ...
          <div className="mt-1 flex items-center">
            ...
            <span className="ml-2 text-xs text-gray-400">{component.file}</span>
          </div>
        </div>
      </div>
      
      {/* Tabs */}
      <div className="px-4 border-b border-gray-700 bg-gray-900">
          ...
          <div>
            {component.description && (
              <div className="mb-6">
                ...
                <p className="text-sm text-gray-200">{component.description}</p>
              </div>
            )}
            
            {component.props && component.props.length > 0 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium text-gray-300 mb-2">Props</h3>
                    ...
                    <tbody className="bg-gray-800 divide-y divide-gray-700">
                      {component.props.map((prop) => (
                        <tr key={prop.name}>
                          ...
                          <td className="px-4 py-2 text-sm text-gray-200 font-mono">
                            {prop.type}
                          </td>
                          <td className="px-4 py-2 text-sm text-gray-200">
                            {prop.required ? (
                              <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-red-900/50 text-red-300 border border-red-700">
                                Required
                              </span>
                            ) : (
                            ...
                            {!prop.required && prop.defaultValue && (
                              <div className="text-xs text-gray-400 mt-1">
                                Default: <code className="px-1 py-0.5 bg-gray-700 rounded font-mono border border-gray-600">{prop.defaultValue}</code>
                              </div>
                            )}
                          </td>
                        </tr>
              ...
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {component.exports.map((exportName) => (
                    <span 
                      key={exportName}
                      className="inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium bg-gray-700 text-gray-200 border border-gray-600"
                    >
                      {exportName}
                    </span>
                  ))}
        ...
        {activeTab === 'connections' && (
          <div>
                ...
                <div className="grid grid-cols-1 gap-2">
                  {uses.map((usedComponent) => (
                    <button
                      ...
                      className="flex items-center p-2 bg-gray-800 rounded-md hover:bg-gray-700 transition-colors text-left border border-gray-700"
                      onClick={() => onSelectComponent(usedComponent.name)}
                    >
                      <TypeBadge type={usedComponent.type} />
                      <span className="ml-2 text-sm font-medium text-gray-200">{usedComponent.name}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {usedBy && usedBy.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-gray-300 mb-2">Used By</h3>
                <div className="grid grid-cols-1 gap-2">
                  {usedBy.map((parentComponent) => (
                    ...
                    >
                      ...
                      <span className="ml-2 text-sm font-medium text-gray-200">{parentComponent.name}</span>
                    </button>
                  ))}
                </div>
              </div>
